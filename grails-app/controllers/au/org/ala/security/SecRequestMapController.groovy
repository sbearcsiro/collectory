...
class SecRequestMapController {
	...
	def list = {
		...
		[requestmapList: SecRequestMap.list(params)]
	}

	def show = {
		def requestmap = SecRequestMap.get(params.id)
		if (!requestmap) {
			flash.message = "SecRequestMap not found with id $params.id"
	...
	def delete = {
		...
		flash.message = "SecRequestMap $params.id deleted."
		redirect(action: list)
	}
	...
	def update = {
		...
		if (requestmap.version > version) {
			requestmap.errors.rejectValue 'version', "requestmap.optimistic.locking.failure",
				"Another user has updated this SecRequestMap while you were editing."
	...
	def create = {
		[requestmap: new SecRequestMap(params)]
	}
	 ...
	 */
	def save = {
		def requestmap = new SecRequestMap(params)
		if (requestmap.save()) {
			authenticateService.clearCachedRequestmaps()
			redirect action: show, id: requestmap.id
		}

      ...
      <table>
        ...
        <tr class="prop">
          ...
          <td valign="top" colspan="3" class="value ${hasErrors(bean: command, field: 'state', 'errors')}">
            <g:select id="state" name="state" from="${command.constraints.state.inList}" value="${command?.state}" valueMessagePrefix="providerGroup.state" noSelection="['': '']"/>
        ...
        <tr>
          <td></td><td colspan="2">Drag pin on map to the exact location of the collection. You may need to zoom in and turn on the hybrid view to locate the building.</td>
        </tr>
